...
export const getTopStories = async (page: number):Promise<Story[]> => {
  const lastStoryNumber: number = page > 0 ? page * 10 : 10
  ...
  return Promise.all(topstories.splice(lastStoryNumber - 10, 10).map(id => fetch(`https://hacker-news.firebaseio.com/v0/item/${id}.json`, {
    next: {
      revalidate: 120
    }
  }).then(res => res.json())))
